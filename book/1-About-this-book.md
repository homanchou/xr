...
This book is about how to use the Phoenix Web Framework (and other building blocks) to create a website that can host a variety of meeting rooms where each room URL is a 3D immersive web experience for games or meetings.  It's not a book that teaches Phoenix or Elixir (the language it's based on), or Typescript (the language Babylon.js is based on).  However we'll go over the design approach and share code snippets along the way so if you follow along from beginning to end you should have a good understanding of how every piece fits together.